...
def monthly_diurnal_graph(
    data: pd.DataFrame,
    observed_diurnal_data: pd.DataFrame,
    field: Field,
    month: int,
    ylim: List[Any],
    output_dir: str = None,
    ...
    **kwargs,
    ...
    if observed_diurnal_data is not None:
        ...
        ax.fill_between(_odf.index, _odf['min'], _odf['max'],
                        color=colors.to_rgba(observed_fill_color, fill_opacity))
        ax.legend([field.id, 'observed'], loc='upper right')

    ax.set_xlim([0, 23])
    ax.set_ylim(ylim)
    ax.fill_between(_df.index, _df['min'], _df['max'], color=colors.to_rgba(fill_color, fill_opacity))
    ...
    if output_dir:
        filename = f'{title}-{chart_id or str(datetime.now().strftime("%Y_%m_%d_%H_%M_%S"))}-{month+1}'
        fig.savefig(f'{output_dir}/{filename}.{output_format}',
                    format=output_format, transparent=True)
        logger(f'Output saved to {filename}')
    if close:
