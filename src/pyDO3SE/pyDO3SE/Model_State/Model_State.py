...
MAX_NUM_OF_CANOPY_LAYERS = 6  # TODO: Where should this be defined?
MAX_NUM_OF_CANOPY_COMPONENTS = 1  # TODO: Where should this be defined?
MAX_NUM_OF_LEAF_POPULATIONS = 3  # TODO: Where should this be defined?
...
class layer_Meteorological_State:
    ...
    micro_O3: float = None        # O3 concentration at layer canopy [ppb]
...
class Whole_Canopy_State:
    ...
    LAI_total: float = None  # sum of layer component SAI
    ...
    rmodel_O3: Resistance_Model = field(default_factory=lambda: Resistance_Model(
        MAX_NUM_OF_CANOPY_LAYERS))  # Must be initialised
...
class Canopy_Population_State:
    ...
    mean_gsto_per_layer: List[float] = field(default_factory=lambda: [None for _ in range(
        MAX_NUM_OF_CANOPY_LAYERS)])        # Leaf stomatal conductance [mmol O3 m-2 PLA s-1]

    # mean Leaf stomatal conductance * LAI [mmol O3 PLA s-1]
    bulk_gsto_per_layer: List[float] = field(
        default_factory=lambda: [None for _ in range(MAX_NUM_OF_CANOPY_LAYERS)])
    ...
    # TODO: Check these in default processes^^^^^^^^^^^^^^^^^^^

    O3up: float = 0  # Stomatal ozone flux (fst) [nmol O3 m-2 PLA s-1]
    O3up_acc: float = 0.0  # Accumulated Stomatal ozone flux [nmol O3 m-2 PLA s-1]
    ...
    fLAI_layer: List[float] = field(default_factory=lambda: np.zeros(MAX_NUM_OF_CANOPY_LAYERS))
...
class Canopy_Component_State:
    ...
    leaf_pop_distribution: List[List[float]] = np.zeros(
        (MAX_NUM_OF_CANOPY_LAYERS, MAX_NUM_OF_LEAF_POPULATIONS))
    ...
    growing_populations: List[bool] = np.zeros(MAX_NUM_OF_LEAF_POPULATIONS)

    # Thermal time difference between leaf pop emergence and current td
    td_dd_leaf_pops: List[float] = np.zeros(MAX_NUM_OF_LEAF_POPULATIONS)
...
class Canopy_Layer_Component_State:
    ...
    fLAI_layer: List[float] = field(default_factory=lambda: np.zeros(MAX_NUM_OF_LEAF_POPULATIONS))
...
class Model_State_Shape:
    ...
    canopy_component_population: List[List[Canopy_Population_State]] = \
        field(default_factory=lambda:
              fill_np_array_with_cls(shape=(MAX_NUM_OF_CANOPY_COMPONENTS, MAX_NUM_OF_LEAF_POPULATIONS),
                                     cls=Canopy_Population_State))

    # Formally ML_t
    canopy_layers: List[Canopy_Layer_State] = \
        field(default_factory=lambda:
              fill_np_array_with_cls(MAX_NUM_OF_CANOPY_LAYERS, Canopy_Layer_State))

    # Formally MC_t
    canopy_component: List[Canopy_Component_State] = \
        field(default_factory=lambda:
              fill_np_array_with_cls(MAX_NUM_OF_CANOPY_COMPONENTS, Canopy_Component_State))

    # Formally MLMC_t
    canopy_layer_component: List[List[Canopy_Layer_Component_State]] = \
        field(default_factory=lambda: fill_np_array_with_cls(
            shape=(MAX_NUM_OF_CANOPY_LAYERS, MAX_NUM_OF_CANOPY_COMPONENTS),
            cls=Canopy_Layer_Component_State,
        ))

    canopy_layer_component_pop: List[List[List[Canopy_Layer_Component_Pop]]] = \
        field(default_factory=lambda: fill_np_array_with_cls(
            shape=(MAX_NUM_OF_CANOPY_LAYERS, MAX_NUM_OF_CANOPY_COMPONENTS, MAX_NUM_OF_LEAF_POPULATIONS),
