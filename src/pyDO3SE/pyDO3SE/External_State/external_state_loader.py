...
from pyDO3SE.error_handling import InputDataError
...
def load_external_state_csv(
    ...
    with open(external_state_file_location, encoding='utf-8-sig') as external_state_file:
        try:
            ...
            print(f"Failed to inport data due to unicode error")
...
def load_external_state_netcdf(
    ...
    use_parallel: bool
        If true uses parallel processing to open mfdatasets
    ...
    assert "time" in variable_map, f"Must include \"time\" in variable_map.json"
    ...
    if multi_file_data:
        ...
        # TODO: May not need file suffix here
        file_filter = f"*{data_filter}*" if data_filter else "*"
        use_dask = True  # open_mfdataset uses dask
        try:
            data = xr.open_mfdataset(f'{data_location}/{file_filter}',
                                     parallel=use_parallel, engine="netcdf4")
        except:
            raise ValueError(
        ...
        try:
            data = xr.open_dataset(data_location, chunks=chunks)
        except:
            raise ValueError(
    ...
    data_processed[time_key].dims[0] == data_processed[variable_map['Ts_C']].dims[0]
...
def get_date_bounds_from_ext_data(
    ...
    **kwargs,
    ...
    ----------
    external_state_file_location : Path
        Path to the external data file (csv or netcdf)
    grid_coords: List[Tuple[int, int]]
    ...
    **kwargs
    ...
    sample_e_state = next(load_external_state(
        ...
        [grid_coords[0]],
        _external_state_options,
    ))
    # TODO: Can this handle when data wraps around end of year?
    start_day = sample_e_state.dd[0]
    end_day = sample_e_state.dd[-1]
    row_count = len(sample_e_state.dd)
    # if _external_state_options.multi_file_data or _external_state_options:
    ...
    if sample_e_state.time and sample_e_state.time[0]:
        start_date = sample_e_state.time[0]
        end_date = sample_e_state.time[-1]
        time_data = pd.date_range(start_date, periods=len(sample_e_state.time), freq="1H")
        # time_data = sample_e_state.time
    ...
    **kwargs
    ...
    if _external_state_options.file_type == FileTypes.CSV:
        ...
        try:
            ...
            raise InputDataError(f"Failed to load external data from \"{external_state_file_location}\"")
        ...
        try:
            loaded_data = load_external_state_netcdf(
                ...
                file_suffix=_external_state_options.file_suffix,
                data_filter=_external_state_options.data_filter,
                chunks=_external_state_options.netcdf_chunks,
            )
        except Exception:
            raise InputDataError(f"Failed to load external data from \"{external_state_file_location}\"")
