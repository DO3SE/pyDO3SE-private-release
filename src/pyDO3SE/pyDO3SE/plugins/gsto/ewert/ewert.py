...
def co2_concentration_in_stomata_iteration(
    ...
    calc_CO2_assimilation_rate=calc_CO2_assimilation_rate,
) -> CO2_loop_State:
...
def co2_concentration_in_stomata_loop(
    ...
    final_state: CO2_loop_State = fp_while(
        ...
        lambda state: co2_concentration_in_stomata_iteration(
            constant_inputs, state, model_options, calc_CO2_assimilation_rate
        ),
        initial_state,
...
def ewert_leaf_pop(
    ...
    g_sv_per_layer = [
        state_out_sun[iL].g_sto * sun_fracs[iL]
        + state_out_shade[iL].g_sto * shade_fracs[iL]
        for iL in range(nL)
    ]
    ...
    A_n = sum(
        [
            layer_lai[iL]
            * (
                state_out_sun[iL].A_n * sun_fracs[iL]
                + state_out_shade[iL].A_n * shade_fracs[iL]
            )
            for iL in range(nL)
        ]
    )
    A_c = sum(
        [
            layer_lai[iL]
            * (
                state_out_sun[iL].A_c * sun_fracs[iL]
                + state_out_shade[iL].A_c * shade_fracs[iL]
            )
            for iL in range(nL)
        ]
    )
    A_j = sum(
        [
            layer_lai[iL]
            * (
                state_out_sun[iL].A_j * sun_fracs[iL]
                + state_out_shade[iL].A_j * shade_fracs[iL]
            )
            for iL in range(nL)
        ]
    )
    A_p = sum(
        [
            layer_lai[iL]
            * (
                state_out_sun[iL].A_p * sun_fracs[iL]
                + state_out_shade[iL].A_p * shade_fracs[iL]
            )
            for iL in range(nL)
        ]
    )
    c_i = sum(
        [
            layer_lai[iL]
            * (
                state_out_sun[iL].c_i * sun_fracs[iL]
                + state_out_shade[iL].c_i * shade_fracs[iL]
            )
            for iL in range(nL)
        ]
    )
    R_d = sum(
        [
            layer_lai[iL]
            * (
                inputs_sun[iL].R_d * sun_fracs[iL]
                + inputs_shade[iL].R_d * shade_fracs[iL]
            )
            for iL in range(nL)
        ]
    )
    ...
    v_cmax_out = sum(
        [
            layer_lai_frac[iL]
            * (
                inputs_sun[iL].V_cmax * LAIsunfrac[iL]
                + inputs_shade[iL].V_cmax * shade_fracs[iL]
            )
            for iL in range(nL)
        ]
    )
    j_max_out = sum(
        [
            layer_lai_frac[iL]
            * (
                inputs_sun[iL].J_max * LAIsunfrac[iL]
                + inputs_shade[iL].J_max * shade_fracs[iL]
            )
            for iL in range(nL)
        ]
    )
