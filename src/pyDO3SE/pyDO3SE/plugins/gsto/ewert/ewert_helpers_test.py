...
def test_calc_ozone_damage_factors():
    ...
    assert isclose(out.fO3_l, 0.375, abs_tol=1e-3)
...
def test_calc_ozone_impact_on_lifespan():
    ...
    assert isclose(out.t_lma_O3, 255.0, abs_tol=1e-3)
...
def test_calc_ozone_impact_on_lifespan_high_ozone():
    ...
    assert isclose(out.t_lma_O3, 544.0, abs_tol=1e-3)
    assert isclose(out.t_lse_limited, 179.52, abs_tol=1e-3)
...
def test_full_hour():
    ...
    assert isclose(f_LS, 0.272796, abs_tol=1e-3)
