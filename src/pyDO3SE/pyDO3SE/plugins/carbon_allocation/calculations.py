...
def calc_net_prod(
    ...
    We use above Clark et al 2011 method which is an expanded version of the same Osborne
    equations.
    ...
    Below Osborne method for reference
    ...
    R_pm = R_dc * (beta + ((c_root + c_stem) / c_leaf))  # maintanance respiration
    R_pg = r_g * (GPP - R_pm)  # Growth Respiration
    R_p = R_pm + R_pg
    NPP = GPP - R_p
    NPP = max(0, NPP)
    return RespirationValues(NPP, GPP, R_pm, R_pg)
...
def calc_carbon_pool_change(
    ...
    https://www.desmos.com/calculator/u3c6pydzyb
...
def daily_carbon_allocation(
    ...
    c_lbrn: float
        brown leaf carbon pool [kg C m^-2]
    a_root : float
    ...
    plant_is_senescing: boolean
        If true then override carbon -> havest at DVI > 1.5 rule
        Instead carbon -> harvest when plant_is_senescing is true
    f_green_leaf: float
    ...
    # TODO: This should be per leaf population
    if DVI > 1.5 or plant_is_senescing:
        c_dist = (1 - f_green_leaf) * c_leaf_out  # : Carbon to be distributed from green leaf
        c_harv_out = c_harv_out + c_dist * (1 - f_brown_leaf)
        c_leaf_out = f_green_leaf * c_leaf_out
        c_lbrn_out = c_lbrn + f_brown_leaf * c_dist
...
def calculate_yield(
    ...
    c_stem: float,
        Stem Carbon [kg C m^-2]
    c_leaf: float,
        Green Leaf Carbon [kg C m^-2]
    c_harv: float,
        Harvest/Grain Carbon [kg C m^-2]
    c_resv: float,
        Stem Reserve Carbon [kg C m^-2]
    c_lbrn: float,
        Brown leaf carbon [kg C m^-2]
    dry_to_wet_biomass : float, optional
        https://edis.ifas.ufl.edu/publication/AG442, by default 1/0.84
    grain_to_ear : float, optional
    ...
    YieldOutputs
        ...
        lbrn_dm: float
            lbrn dry matter [?]
        total_leaf_dm: float
            total_leaf dry matter [?]
        straw_dm: float
            straw dry matter [?]
        ear_dm: float
            ear dry matter [?]
        aboveground_dm: float
            above ground dry matter [?]
        belowground_dm: float
    ...
    """
    kg_to_g = 1000  # experimental outputs are in grams
    f_c = 1 / 0.5

    stem_dm = (c_stem + c_resv) * kg_to_g * f_c
    leaf_dm = c_leaf * kg_to_g * f_c
    lbrn_dm = c_lbrn * kg_to_g * f_c
    total_leaf_dm = (c_leaf + c_lbrn) * kg_to_g * f_c
    straw_dm = (c_stem + c_resv + c_leaf + c_lbrn) * kg_to_g * f_c
    ear_dm = c_harv * kg_to_g * f_c
    aboveground_dm = straw_dm + ear_dm
    belowground_dm = c_root * kg_to_g * f_c
    grain_dm = c_harv * kg_to_g * grain_to_ear * f_c
    harvest_index = grain_dm / aboveground_dm
    yield_ha = (c_harv * f_c * dry_to_wet_biomass * kg_to_g) * grain_to_ear
