...
from pyDO3SE.Pipelines.default_processes import raise_error
...
def calculate_phenology_stages_process(config: Config_Shape, iLC: int):
    ...
    return [
        ...
        Process(
            ...
            gate=config.Land_Cover.phenology_options.zero_day in [ZeroDayOptions.SOWING, ZeroDayOptions.EMERGENCE],
...
def layer_pop_trim(config: Config_Shape) -> List[Process]:
    ...
    def trim_state(state, nLC, nL, nP):
        return replace(
            state,
            canopy_component_population=[[state.canopy_component_population[iLC][iP] for iP in range(nP)] for iLC in range(nLC)],
            canopy_layers=[state.canopy_layers[iL] for iL in range(nL)],
            canopy_component=[state.canopy_component[iLC] for iLC in range(nLC)],
            canopy_layer_component=[[state.canopy_layer_component[iL][iLC] for iLC in range(nLC)] for iL in range(nL)],
        )
