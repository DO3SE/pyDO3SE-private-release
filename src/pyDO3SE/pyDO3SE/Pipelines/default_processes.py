...
def calc_fst_leaf_acc_hour_process(iLC: int, iP: int) -> Process:
    ...
    Note that the use of the sowing_to_emerge_knee restricts uptake of O3 until
    ...
    To set the knee value you manually add a thermal time value to
        Config.Land_Cover.parameters[iLC].phenology.key_lengths_flag_leaf_td.plant_emerg_to_leaf_emerg_knee

    """

    return Process(
        func=lambda O3up, O3up_acc, has_emerged, leaf_emerg_to_leaf_fst_acc, td: 0 if (
            not has_emerged or td < leaf_emerg_to_leaf_fst_acc) else O3up + O3up_acc,
