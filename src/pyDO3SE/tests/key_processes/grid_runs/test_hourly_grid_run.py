    ...
    class TestSingleFileRangeCombined(Base):
        ...
        def test_should_have_merged_all_files(self):
            ...
            assert len(output_files) == 1, (
                f"Model should have only produced 1 nc file but {len(output_files)} files"
            )
    ...
    class TestSingleFileRangeGroupByYear(Base):
        ...
        def test_should_have_merged_all_files(self):
            ...
            assert len(output_files) == 2, (
                f"Model should have only produced 2 nc files but {len(output_files)} files"
            )
...
class TestIntegrated:
    ...
    def test_grid_run(self):
        ...
        logger_main = Logger(
            log_level, log_to_file=None, set_as_default=True, write_mode="w", flush_per_log=True
        )
        ...
        run_grid_model.main_grid_run(
            ...
            parallel=False,
            parallel_args=setup_grid_model.ParallelArgs(8, 0, 0.01),
