...
def calc_leaf_pops_per_layer(
    ...
    """
    # TODO: Check this is summing on correct axis etc
    prev_leaf_lais = [sum([prev_leaf_pops_per_lai[iL][iP] for iL in range(nL)]) for iP in range(nP)]  # sum of population over all layers
...
def get_growing_populations_range_from_config(
    ...
    td_emerge_leaf_pops = [[tdd - t_emerge for t_emerge in leaf_population_t_emerg] for tdd in td_dd_plant_emerge]
