...
from do3se_phenology.latitude_function import lat_function_winter_wheat
...
def get_sowing_day_from_config(
    ...
    if model_config.sowing_day_method == SowingDateMethods.LATITUDE:
        ...
        sowing_day = lat_function_winter_wheat(model_config.latitude)
        ...
        sowing_day = species_config.key_dates.sowing
...
def phenology_from_season_fraction(
    ...
    if model_config.zero_day == ZeroDayOptions.ASTART:
        Astart = get_astart_from_config(species_config)
...
def calc_key_dates(
    ...
    species_config_out = deepcopy(species_config)
    sowing_dd = species_config_out.key_dates.sowing
...
def validate_config(
    ...
    if species_config.f_Astart + species_config.f_fphen_1_ets + species_config.f_fphen_5_ets != 1.0:
        ...
        raise ConfigError(f"f_Astart + f_fphen_1_ets + f_fphen_5_ets != 1.0 Value is: {val}")
    ...
    if species_config.key_lengths_flag_leaf_td.plant_emerg_to_leaf_emerg < 0:
        raise ConfigError(f"flag emerges before plant emerges. Make sure f_t_lem > f_Astart")
    return True
...
def process_phenology_config(
    ...
    # TODO: We need to get all fractions and parameters from kwargs rather than using defaults

    # if output_time_type != TimeTypes.THERMAL_TIME and phenology_method in [
    #     PhenologyMethods.FPHEN_THERMAL_TIME,
    #     PhenologyMethods.DEVELOPMENT_INDEX,
    # ]:
    #     raise ValueError(f"Must use thermal time for {phenology_method} method!")

    # TODO: Get preset params here
    ...
    output_time_type: TimeTypes = TimeTypes.THERMAL_TIME
    ...
    if phenology_method == PhenologyMethods.FPHEN_THERMAL_TIME:
        generated_configs = phenology_from_fphen_td_int(
            ...
            output_time_type=output_time_type,
        )
    ...
    if not validate_config(final_species_config):
