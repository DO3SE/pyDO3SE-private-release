...
def plot_ewert_phenology_data(
    ...
    **kwargs,
    ...
    ax.axvline(t_sgs + t_Astart + t_lem + t_lma, linestyle='dotted', color=COLOR)
    ax.axvline(t_sgs + t_Astart + t_lem + t_lep, linestyle='dotted', color=COLOR)
...
def plot_phenology_from_config(
    ...
    # ===== PLOTS ===== #
    fig, axss = plt.subplots(ncols=2, nrows=nrows, figsize=(10 * 2, nrows * 5), dpi=120)
    axs_td = [ax[0] for ax in axss] if nrows > 1 else [axss[0]]
    axs_dd = [ax[1] for ax in axss] if nrows > 1 else [axss[1]]
