...
from do3se_phenology.config import DayFphenPlf, FPhenMethods, LeafFPhenMethods, PlantEmergeMethod, SowingDateMethods, Wheat
...
example_data = dict(
    ...
    dd=dd,
    leaf_fphen=np.interp(np.arange(0, 2200 + 20, 20 / 24), np.array([0, 1080, 1081, 1240, 1340, 1680, 2000, 2000]) - (t_astart - example_SGS_t), [
...
def default_expected_species_output(nP): return replace(
    Wheat,
    PRESET=SpeciesPresets.WHEAT_SPRING,
    f_tt_emr=0.05,
    f_tt_veg=700 / 2000,
...
default_expected_species_output_astart = replace(
    ...
    key_lengths_leaf_td=PhenologyLeafKeyLengths(
        tl=1900,
        tl_em=980 - 280,  # TODO: Should equal the td between plant emerge and flag emerge divided by no of pops
        tl_ma=1200,
        tl_ep=round(1200 * (0.31 / 0.46), 4),
        tl_se=round(1200 * (0.15 / 0.46), 4),
        leaf_f_phen_e=None,
...
class SwitchBoardTestBase():
    ...
    def _run(self, model_config=None, species_config=None, external_data=None, nP=nP):
        return process_phenology_config(
            model_config=model_config or self.model_config,
            species_config=species_config or self.species_config,
            external_data=external_data or self.external_data,
    ...
    def test_defines_all_key_dates_leaf(self):
        ...
        assert out_species_config.key_lengths_leaf_td == self.expected_species_config.key_lengths_leaf_td
    ...
    def test_defines_all_key_intervals(self):
        ...
        assert out_species_config.leaf_fphen_intervals == self.expected_species_config.leaf_fphen_intervals
    ...
    def test_values_should_be_sensible(self):
        ...
        if self.model_config.time_type == TimeTypes.THERMAL_TIME:
            assert out_species_config.key_lengths_leaf_td.tl <= out_species_config.key_lengths_td.emerg_to_end
    ...
    def test_should_work_with_multiple_leaf_pops(self):
        ...
        if self.model_config.time_type != TimeTypes.THERMAL_TIME:
            return
        emerge_to_flag_emerg = self.expected_species_config.key_lengths_flag_leaf_td.plant_emerg_to_leaf_emerg
...
class TestSwitchBoardDayPLF(SwitchBoardTestBase):
    ...
    model_config = ModelConfig(
        ...
        sowing_day_method=SowingDateMethods.LATITUDE_SPRING_EUROPE,
        latitude=50,
    ...
    expected_species_config = replace(
        ...
        key_lengths=replace(
            ...
            emerg_to_end=197-105 - 10,
            emerg_to_astart=125,
        ),
        key_lengths_leaf=replace(
            deepcopy(species_config.key_lengths_leaf),
            # plant_emerg_to_leaf_emerg=0,

        ),
        key_lengths_flag_leaf=replace(
            deepcopy(species_config.key_lengths_flag_leaf),
            plant_emerg_to_leaf_emerg=125,
            leaf_emerg_to_astart=0,
            astart_to_senescence=167,
        ),
    )
...
class TestSwitchBoardDayPLFPlantOnly(TestSwitchBoardDayPLF):
    model_config = replace(
        deepcopy(TestSwitchBoardDayPLF.model_config),
    )
    species_config = replace(
        deepcopy(TestSwitchBoardDayPLF.species_config),
        leaf_f_phen_method=LeafFPhenMethods.F_PHEN,
        key_lengths=replace(
            deepcopy(TestSwitchBoardDayPLF.species_config.key_lengths),
            sowing_to_astart=None,
        ),
        day_fphen_plf=replace(
            deepcopy(TestSwitchBoardDayPLF.species_config.day_fphen_plf),
            leaf_f_phen_a=None,
            leaf_f_phen_b=None,
            leaf_f_phen_c=None,
            leaf_f_phen_1=None,
...
def test_plot_overlays():
    ...
    plot_phenology_from_config(
        ...
        nP=nP,
        external_data=example_data,
        output_location="tests/outputs/phenology.png",
        plot_dd=True,
    )
