...
@pytest.mark.parametrize(['sinB', 'P', 'PAR', 'output'], [
    [1.8, 1.9, 10.0, (0.1402, 9.8597)],
    [0.384, 91.2, 300.0, (204.9826, 95.01739)],
    # [0.0001, -999.99, 24.0, (204.9826, 95.01739)],
    [0.0001, 109, 24.0, (0.0, 24.0)],
])
def test_calc_Idrctt_Idfuse(sinB, P, PAR, output):
    """Test the output of calc_Idrctt_Idfuse."""
    Idrctt, Idfuse, _ = calc_Idrctt_Idfuse(
        sinB=sinB,
        P=P,
        PAR=PAR,
    )
    assert isclose(Idrctt, output[0], abs_tol=1e-4)
    assert isclose(Idfuse, output[1], abs_tol=1e-4)
