...
from math import exp, isnan, log, pi, sqrt, isclose
...
def calc_monin_obukhov_length(
    ...
    # max(Hd, 0.000000000001) # Limit turned off to match EMEP
    Hd_f = Hd if Hd != 0 else 0.00000000001
...
def calc_ustar_and_L(
    ...
    if ustar_ref_in is not None:
        ...
        if isnan(L):
            raise ValueError(f"monin_obukhov_length failed! Input is \
...
def calc_windspeed(
    ...
    # NOTE: moved these to parameter setup
...
def calc_windspeed_ustar_in(
    ...
    # Find windspeed at izR, over reference canopy
    u_i = estimate_windspeed(u, z_u_a, izr, u_z0_a, d, 1 / L) if L is not None \
        else velocity_from_ustar(ustar_in, (izr - u_d_a), u_z0_a)
    u_i_lim = max(min_windspeed, u_i)

    ustar_lim = max(MIN_USTAR, ustar_in)
...
def calc_layer_windspeed(
    ...
    if o_top_chamber:
        ...
        if w <= 0:
            return u_at_canopy_top
        assert w is not None, "Must supply leaf width(w) for calc_layer_windspeed"
        assert h is not None, "Must supply canopy height(h) for calc_layer_windspeed"
        ...
        # TODO: Added lim here to avoid division by 0. Is this correct?
        h_lim = max(1.000001, h)
        # TODO: why do we `-1` here?
        u_z = u_at_canopy_top * exp(a * (z / h_lim - 1))
        return u_z
