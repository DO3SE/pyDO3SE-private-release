...
def calc_PET_gsto_An(
    bulk_gsto: float,
    f_SW: float,
) -> float:
    """Calculate the potential gsto limited by f_SW fractions.

    Parameters
    ----------
    bulk_gsto: float
        mean_gsto * lai[mmol O3 m-2 PLA s-1]

    """
    Gsto_PEt = bulk_gsto / f_SW
    Rsto_PEt = calc_Rsto(Gsto_PEt)
    return Rsto_PEt


PenmanMonteithHourlyOutput = namedtuple(
    'PenmanMonteithHourlyOutput', 'Ei_hr Et_hr PEt_hr Es_hr Eat_hr, Ei_acc Et_acc PEt_acc Es_acc Eat_acc')
...
def penman_monteith_hourly(
    ...
    return PenmanMonteithHourlyOutput(
        Ei_hr=Ei_hr,
        Et_hr=Et_hr,
...
def multi_layer_r_model_to_single_H20(
    ...
    # TODO: We need a different Ra in the resistance model here
    rmodel_H20 = Resistance_Model(1, Ra_canopy_to_izr=Ra, Rb=Rb, Rinc=[Rinc], Rext=[
