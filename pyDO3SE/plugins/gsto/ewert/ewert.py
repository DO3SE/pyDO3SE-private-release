...
class ModelOptions:
    ...
    f_VPD_method: FVPDMethods
        If "leuning" or "Danielsson" then f_VPD is calculated internally else should be provided as input
...
def ewert_leaf_pop(
    ...
    if sum(layer_lai_frac) == 0:
        return Output_Shape(
            g_sv=None,
            g_sv_per_layer=[0 for _ in range(nL)],
            A_n=0,
            A_c=0,
            A_j=0,
            A_p=0,
    ...
    return Output_Shape(
        g_sv=None,  # Remove as does not make sense
