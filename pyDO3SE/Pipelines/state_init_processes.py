...
from pyDO3SE.Model_State.Model_State import (
    MAX_NUM_OF_CANOPY_COMPONENTS,
    MAX_NUM_OF_CANOPY_LAYERS,
    MAX_NUM_OF_LEAF_POPULATIONS,
    Model_State_Shape,
)
...
def soil_moisture_setup(config: Config_Shape) -> List[Process]:
    return [
        ...
        Process(
            ...
            gate=config.soil_moisture.source == 'P-M',
...
def layer_pop_trim(config: Config_Shape) -> List[Process]:
    ...
    def trim_state(state, nLC, nL, nP):
        assert nP <= MAX_NUM_OF_LEAF_POPULATIONS, f"Must set environment variable MAX_NUM_OF_LEAF_POPULATIONS={nP}"
        assert nL <= MAX_NUM_OF_CANOPY_LAYERS, f"Must set environment variable MAX_NUM_OF_CANOPY_LAYERS={nL}"
        assert nLC <= MAX_NUM_OF_CANOPY_COMPONENTS, f"Must set environment variable MAX_NUM_OF_CANOPY_COMPONENTS={nLC}"
