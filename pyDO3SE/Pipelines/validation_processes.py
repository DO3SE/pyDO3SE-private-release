...
def assert_greater_than(amount, strict=True):
    ...
    def _assert_greater_than(**args):
        ...
        try:
            ...
            assert min(value) >= amount, f"Failed to assert {name} is greater than: {amount}. Min value is {min(value)}"
        except TypeError:
...
def validate_external_state(config: Config_Shape) -> List[Process]:
    assert_defined_processes = [
        Process(
            func=assert_list_defined, external_state_inputs=lambda e_state, row_index: [
        ...
        Process(
            func=assert_greater_than(0, strict=False), external_state_inputs=lambda e_state, row_index: [
                I(e_state.VPD, as_="VPD")]
        ),
    ]
...
def setup_validation(config: Config_Shape) -> List[Process]:
