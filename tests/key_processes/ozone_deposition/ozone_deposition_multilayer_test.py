...
class TestRunAndCompare:
    ...
    def test_should_gradually_decrease_ozone_from_top_of_canopy_to_bottom(self, runid):
        ...
        assert all([b * 0.01 < a < b * 0.9 for a, b in zip(micro_O3_iL_0, micro_O3_iL_1)])
        assert all([b * 0.01 < a < b * 0.9 for a, b in zip(micro_O3_iL_1, micro_O3_iL_2)])
        assert all([b * 0.01 < a < b * 0.9 for a, b in zip(micro_O3_iL_2, canopy_top_o3)])
        # bottom layer should be between 1% and 70% of the top layer
        assert all([b * 0.01 < a < b * 0.7 for a, b in zip(micro_O3_iL_0, canopy_top_o3)])
