...
def phenology_from_legacy_day_plf(
    ...
    if species_config.leaf_f_phen_method in [
        ...
        assert astart_to_end >= species_config.day_fphen_plf.leaf_f_phen_1 + species_config.day_fphen_plf.leaf_f_phen_2, "Astart to end must be greater than leaf_f_phen_1 + leaf_f_phen_2"
...
def get_sowing_day_from_config(
    ...
    try:
        if model_config.sowing_day_method == SowingDateMethods.LATITUDE:
            ...
            sowing_day = int(lat_function(model_config.latitude, species_config.lat_f_k,
                             species_config.lat_f_b, species_config.lat_f_c))
...
def phenology_from_season_fraction(
    ...
    leaf_intervals = get_leaf_td_intervals_f(
        ...
        f_t_lep=species_config.f_t_lep,
        f_t_lse=species_config.f_t_lse,
        f_tt_fst_acc=species_config.f_tt_fst_acc,
    )
    # SET PARAMS
    ...
    species_config_out.key_lengths_td.sowing_to_fst_acc = canopy_td.t_fphen_a
    ...
    species_config_out.key_lengths_flag_leaf_td.plant_emerg_to_leaf_emerg = (
        species_config_out.key_lengths_td.emerg_to_astart - leaf_intervals.t_lem)
    species_config_out.key_lengths_flag_leaf_td.leaf_emerg_to_leaf_fst_acc = leaf_intervals.t_fst_acc
...
def phenology_from_fphen_td_int(
    ...
    key_lengths_flag_leaf = replace(
        ...
        tl_se=leaf_intervals.t_lse,
        plant_emerg_to_leaf_emerg=emerg_to_flag_emerg,
        leaf_emerg_to_leaf_fst_acc=leaf_intervals.t_fst_acc,
    )
...
def validate_config(
    ...
    try:
        ...
        if model_config.time_type == TimeTypes.THERMAL_TIME:
            if not isclose(species_config.f_Astart + species_config.f_fphen_1_ets + species_config.f_fphen_5_ets, 1.0, abs_tol=1e-5):
            ...
            if not isclose(species_config.f_tt_emr + species_config.f_tt_rep + species_config.f_tt_veg, 1.0, abs_tol=1e-5):
