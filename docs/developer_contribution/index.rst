...
This contains guidance for developers working directly on the do3se_phenology code. Also ensure you familiarise yourself with the end user documentation.
...
- Branches should not be used to store alternate versions of the model unless they are assumed to be static releases that will not be updated. This avoides trying to keep track of many different versions. Instead you should attempt to use the process runner to create an alternative set of default processes.
...
The processes that are run by default are located in `do3se_phenology/Defaults/default_processes.py`
...
::
   ...
   |-- do3se_phenology/ => The do3se_phenology package
   |   |-- Config/ => Model config
   ...
   |   |-- plugins/ => do3se_phenology modules/plugins
   |-- do3se_phenology_gui/ => do3se_phenology gui package
   |-- requirements/ => Contains the requirement.txt familiarise
   |-- test/ => functional tests
   |-- vendor/ => contains unreleased python packages that are required by do3se_phenology